...
from msconsparser.libs.edifactmsconsparser.wrappers.segments import SegmentGroup, SegmentUNH, NachrichtenKennung, StatusDerUebermittlung
...
class UNHSegmentConverter(SegmentConverter[SegmentUNH]):
    ...
    def _convert_internal(
        ...
        nachrichten_kennung = MSCONSUtils.split_components(element_components[2])
        allgemeine_zuordnungsreferenz = element_components[3] if len(element_components) > 3 and len(
            element_components[3]) > 0 else None
        status_der_uebermittlung_details = MSCONSUtils.split_components(element_components[4]) if len(
