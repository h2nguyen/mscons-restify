...
def test_parse_mscons_raw_format_with_specific_una(client: TestClient):
    """Test that the /parse-raw-format endpoint returns the expected response."""
    # Read the sample MSCONS data from a file
    mscons_file_path = "samples/mscons-message-example-una-spec.txt" \
        if os.path.exists("samples/mscons-message-example-una-spec.txt") \
        else "tests/samples/mscons-message-example-una-spec.txt"
    with open(mscons_file_path, "r") as f:
        mscons_data = f.read()

    # Read the expected response from the JSON file
    expected_response_path = "samples/mscons-message-example-una-spec-expected-response.json" \
        if os.path.exists("samples/mscons-message-example-una-spec-expected-response.json") \
        else "tests/samples/mscons-message-example-una-spec-expected-response.json"
    with open(expected_response_path, "r") as f:
        expected_response = json.load(f)
